...
## 八、樣本擴大結果（2026-09-18，回應「這是死透還是樣本不夠」的追問）

上一節結尾寫「排在動能之前，因為訊號方向未被證偽」——這句話現在可以驗證了。SEC EDGAR 的 8-K 申報資料是免費公開 API（不需要新的付費資料源），把 `data/filings/8k/` 從原本 20 檔（跟微結構訊號同一批半導體/大型科技股，彼此高度相關）擴大到 `data/history/` 現成的 592 檔股票（535 檔成功取得，57 檔沒有——全部是 ETF 或外國私人發行人，本來就不申報 8-K，不是抓取失敗）。用 `scripts/refresh_event_data.py --symbols <535檔> --since 2018-01-01 --skip-news --skip-calendar` 抓取，SEC EDGAR 免費 API，約 8 分鐘完成，零錯誤。

用完全相同的 `scripts/run_v2_8k_drift_phase0.py`（未改動任何邏輯）重跑：

| Universe | 事件數 | Top-bottom 價差 t-stat | 判定 |
|---|---|---|---|
| 20 檔（原始） | 554 | **+0.84** | NO-GO（方向對，疑似樣本不夠） |
| **535 檔（本次擴大）** | **2,748** | **-0.47** | **NO-GO（收斂到零，且變號）** |

**這正是動能測試在第六節看到的同一種模式**：樣本從小、集中的一群相關股票擴大到分散的大樣本後，價差不但沒有變顯著，反而**收斂到接近零、方向還翻轉了**——這是「原本的正訊號是 20 檔高度相關的半導體/科技股的雜訊或共同 beta，不是真正的申報後漂移」的證據型態，跟「樣本不夠大蓋住了真訊號」（擴大後應該同方向收斂變顯著）是相反的證據型態。

**結論更新**：8-K 事件漂移不再是「還沒被證偽的方向」，現在跟動能一樣是「用夠大的樣本測過、收斂到零」。截至本文件此版本，記錄的所有訊號候選（動能：中型股/大型股/合併；8-K 漂移：20 檔/535 檔）全部 NO-GO，且沒有一個是「疑似樣本不夠」——兩個機制都在擴大樣本後更明確地指向零。本 repo 現有資料基礎設施（價格 + SEC 申報）能便宜測試的訊號類型已經測完。再往下如果要繼續找新的 alpha 訊號，需要的是本文件第七節列過、目前不可行的資料源之一（真實 EPS 驚喜數字、選擇權鏈歷史、更長的新聞情緒歷史）——那是一個新的、需要付費資料源或長期資料工程的計畫，不是用現有基礎設施能再擠出來的東西。

_2026-08-25 建立，2026-09-11 依 2026 年動能因子文獻複查修正 + Phase 0 實測結果 + Option D（8-K 事件漂移）Phase 0 結果，2026-09-18 加入 535 檔樣本擴大結果_
